<div x-data="{ modalReady: false, passwordVisible: false, confirmationVisible: false }"
    ...
    <x-modal id="user-modal" wire="isOpen" z-index="z-[70]" size="2xl" scrollable center x-on:close="$wire.close()">
            ...
            <form wire:submit="save" autocomplete="off" class="space-y-5">
                ...
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-12">
                    <div class="col-span-12 min-w-0 sm:col-span-6">
                        ...
                        <div class="flex items-center gap-2">
                            <input id="modal-ide" wire:model="ide" type="text" autocomplete="off" wire:loading.attr="disabled" wire:target="lookupPerson" class="h-10 min-w-0 flex-1 rounded-lg border border-gray-300 bg-transparent px-3 text-sm text-gray-800 outline-none focus:border-brand-500 dark:border-gray-700 dark:text-white/90 disabled:cursor-not-allowed disabled:bg-gray-100 dark:disabled:bg-white/[0.03]" />
                            @if($userId === null)
                    ...
                    <div class="col-span-12 min-w-0 sm:col-span-6 sm:col-start-1"><x-input id="modal-name" wire:model="name" type="text" autocomplete="off" wire:loading.attr="disabled" wire:target="lookupPerson" :label="__('Nombre')" /></div>
                    <div class="col-span-12 min-w-0 sm:col-span-6 sm:col-start-7"><x-input id="modal-lastname" wire:model="lastname" type="text" autocomplete="off" wire:loading.attr="disabled" wire:target="lookupPerson" :label="__('Apellidos')" /></div>
                    <div class="col-span-12 min-w-0"><x-input id="modal-email" wire:model="email" type="email" autocomplete="off" wire:loading.attr="disabled" wire:target="lookupPerson" :label="__('Correo electrónico')" /></div>
                    <div class="col-span-12 min-w-0 sm:col-span-6">
                        <label for="modal-password" class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Contraseña') }}</label>
                        <div class="relative">
                            <input id="modal-password" wire:model="password" :type="passwordVisible ? 'text' : 'password'" autocomplete="new-password" wire:loading.attr="disabled" wire:target="lookupPerson" {{ $userId === null ? 'required' : '' }} class="h-10 w-full rounded-lg border border-gray-300 bg-transparent px-3 pr-11 text-sm text-gray-800 outline-none focus:border-brand-500 dark:border-gray-700 dark:text-white/90 disabled:cursor-not-allowed disabled:bg-gray-100 dark:disabled:bg-white/[0.03]" />
                    ...
                    <div class="col-span-12 min-w-0 sm:col-span-6">
                        ...
                        <div class="relative">
                            <input id="modal-password-confirmation" wire:model="password_confirmation" :type="confirmationVisible ? 'text' : 'password'" autocomplete="new-password" wire:loading.attr="disabled" wire:target="lookupPerson" class="h-10 w-full rounded-lg border border-gray-300 bg-transparent px-3 pr-11 text-sm text-gray-800 outline-none focus:border-brand-500 dark:border-gray-700 dark:text-white/90 disabled:cursor-not-allowed disabled:bg-gray-100 dark:disabled:bg-white/[0.03]" />
                ...
                <div class="flex justify-end gap-3 pt-3">
                    <button type="button" x-on:click="$tsui.close.modal('user-modal')" wire:click="close" class="rounded-lg border border-gray-300 px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.03]">{{ __('Cancelar') }}</button>
