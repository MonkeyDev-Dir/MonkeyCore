<section>
    ...
    <div class="mt-4 space-y-4">
        @forelse($groups as $group)
            <div wire:key="weekly-backup-group-{{ $group['date']->format('Y-m-d') }}">
                <h4 class="mb-2 text-xs font-semibold text-gray-500 dark:text-gray-400">{{ $group['date']->format('d/m/Y') }}</h4>
                ...
                <x-table :headers="$headers" :rows="$group['backups']">
                    ...
                    @interact('column_generated_at', $backup)
                        {{ $backup->generated_at?->format('H:i') }}
                    @endinteract
        ...
        @empty
            <div class="rounded-xl border border-gray-200 px-4 py-8 text-center text-sm text-gray-500 dark:border-gray-800 dark:text-gray-400">
                {{ __('No hay respaldos generados esta semana.') }}
            </div>
        @endforelse
    </div>
</section>
