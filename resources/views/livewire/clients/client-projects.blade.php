            ...
            @foreach($client->projects as $project)
                <article wire:key="client-project-{{ $project->id }}" x-data="{ expanded: false }" class="group relative rounded-lg border border-gray-200 bg-white p-3 transition hover:border-brand-400 hover:shadow-sm dark:border-gray-700 dark:bg-gray-900 dark:hover:border-brand-500">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="absolute right-3 top-3 flex items-center gap-1"><button type="button" x-data x-on:click="window.dispatchEvent(new CustomEvent('open-project-credential-create', { detail: { clientCode: '{{ $clientCode }}', projectId: {{ $project->id }} } }))" class="rounded-lg p-2 text-gray-400 hover:bg-gray-100 hover:text-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500/40 dark:hover:bg-gray-950" aria-label="{{ __('Agregar credencial') }}" title="{{ __('Agregar credencial') }}"><i data-lucide="key-round" class="h-4 w-4" aria-hidden="true"></i></button><x-common.edit-button :label="__('Editar proyecto')" x-data x-on:click="window.dispatchEvent(new CustomEvent('open-project-edit', { detail: { clientCode: '{{ $clientCode }}', projectId: {{ $project->id }} } }))" class="static" /></div>
                    </div>
                    @if($project->credentials->isNotEmpty())<div class="mt-4 border-t border-gray-100 pt-3 dark:border-gray-800"><div class="flex items-center justify-between gap-3"><h4 class="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400"><i data-lucide="key-round" class="h-4 w-4"></i>{{ __('Credenciales registrados') }}</h4></div>@foreach($project->credentials as $credential)<div x-data="{ visible: false }" class="mt-2 rounded-md bg-gray-50 p-2.5 text-xs dark:bg-gray-800/60"><div class="flex items-center justify-between gap-2"><span class="font-medium text-gray-700 dark:text-gray-200">{{ $credential->name }}</span><button type="button" x-data x-on:click="window.dispatchEvent(new CustomEvent('open-project-credential-edit', { detail: { clientCode: '{{ $clientCode }}', projectId: {{ $project->id }}, credentialId: {{ $credential->id }} } }))" class="text-gray-400 hover:text-brand-500" aria-label="{{ __('Editar credencial') }}"><i data-lucide="pencil" class="h-3.5 w-3.5"></i></button></div><div class="mt-1 space-y-1 text-gray-600 dark:text-gray-400">@if($credential->login_url)<a href="{{ $credential->login_url }}" target="_blank" rel="noopener noreferrer" class="block truncate text-brand-500 hover:underline">{{ $credential->login_url }}</a>@endif<div><span class="font-medium">{{ __('Usuario') }}:</span> {{ $credential->username ?: __('Sin información') }}</div><div class="flex items-center gap-2"><span class="font-medium">{{ __('Contraseña') }}:</span><span x-text="visible ? @js($credential->password ?? __('Sin información')) : '••••••••'">••••••••</span><button type="button" x-on:click="visible = !visible" class="text-brand-500 hover:text-brand-600" x-text="visible ? '{{ __('Ocultar') }}' : '{{ __('Ver') }}'"></button></div></div></div>@endforeach</div>@endif
