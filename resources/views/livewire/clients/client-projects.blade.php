    ...
    @else
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            @foreach($client->projects as $project)
                <article wire:key="client-project-{{ $project->id }}" x-data="{ expanded: false }" class="group relative rounded-xl border border-gray-200 bg-gray-50/70 p-5 transition hover:border-brand-300 hover:shadow-sm dark:border-gray-800 dark:bg-gray-900/40 dark:hover:border-brand-500/50">
                    <div class="flex items-start gap-3">
                        <div class="flex min-w-0 items-start gap-3"><span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-brand-50 text-brand-500 dark:bg-brand-500/10 dark:text-brand-400"><i data-lucide="folder-kanban" class="h-5 w-5" aria-hidden="true"></i></span><div class="min-w-0"><h3 class="truncate font-semibold text-gray-800 dark:text-white/90">{{ $project->name }}</h3>@if($project->code)<span class="mt-1 inline-flex items-center gap-1 rounded-md bg-brand-50 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wide text-brand-600 dark:bg-brand-500/10 dark:text-brand-400"><i data-lucide="hash" class="h-3 w-3" aria-hidden="true"></i>{{ $project->code }}</span>@endif</div></div>
                    ...
                    @if($project->description)
                        <div x-show="!expanded" class="mt-4 line-clamp-3 text-sm leading-6 text-gray-600 dark:text-gray-400">{{ $project->descriptionPreview() }}</div>
                        <div x-cloak x-show="expanded" class="prose prose-sm mt-4 max-w-none text-gray-600 dark:prose-invert dark:text-gray-400">{!! $project->description !!}</div>
                        ...
                        @endif
                    @else
                        <p class="mt-4 text-sm italic text-gray-400 dark:text-gray-500">{{ __('Sin descripción') }}</p>
                    @endif
                </article>
            @endforeach
        </div>
